exports.run = async (client, message, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "775797087606014013")return message.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Ağzına sıçtığım sen kimsinde Kadire söversin!')
}})
  if(member.id === "696061560912740513")return message.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcıma sövemezsin!!`)
}})
  if(member.id === "908969675352846376")return message.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Arma 'ya' sövemezsin!!`)
}})
