...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
   ...
   .setColor('RANDOM')
   .setTimestamp()
   .setDescription("Oynamak istediğin kişiyi etiketlemelisin!"))

  if (opponent.bot) return message.reply(
    new Discord.MessageEmbed()
   .setColor('RANDOM')
   .setTimestamp()
   .setDescription('Botlar ile oynayamazsın!'));
  if (opponent.id === message.author.id) return message.reply(
    new Discord.MessageEmbed()
   .setColor('RANDOM')
   .setTimestamp()
   .setDescription('Kendin ile düello Atamazsın'));
		if (this.fighting.has(message.channel.id)) return message.reply(
      new Discord.MessageEmbed()
     .setColor('RANDOM')
     .setTimestamp()
     .setDescription('Kanal başına sadece bir düello meydana gelebilir.'));
		...
		try {
			if (!opponent.bot) {
                await message.channel.send(
                 ...
                 .setColor('RANDOM')
                 .setTimestamp()
                 .setDescription(`${opponent}, düello isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`));
           ...
           .setColor('RANDOM')
           .setTimestamp()
           .setDescription(`Düello kabul edilmedi...`));
				}
			}
			let userHP = 500;
			let oppoHP = 500;
           ...
           .setDescription(stripIndents`
             ...
             .setColor('RANDOM')
             .setTimestamp()
             .setDescription(`Üzgünüm ama, süre doldu!`));
						reset();
						continue;
					}
           ...
           .setDescription(`${user}, kendisini süper kalkan ile savundu!`));
             ...
             .setColor('RANDOM')
             .setTimestamp()
             .setDescription(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterli miktarda toplayamadığın için ulta güç kullanamadın!`));
					}
					reset();
				} else if (choice === 'kaç') {
					await message.channel.send(
           ...
           .setColor('RANDOM')
           .setTimestamp()
           .setDescription('Ne yapmak istediğini anlamadım.'));
				}
			}
			this.fighting.delete(message.channel.id);
       ...
       .setColor('RANDOM')
       .setTimestamp()
       .setDescription(`Oyun bitti! Tebrikler, **${winner}** kazandı! \n**${message.author.username}**: ${userHP} :heartpulse: \n**${opponent.username}**: ${oppoHP} :heartpulse:`));
		} catch (err) {
			this.fighting.delete(message.channel.id);
			throw err;
		}
